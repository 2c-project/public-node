...
public class CryptoUtils {
    ...
    public static String signatureToString(byte[][] signature) {
        return Utils.bytesToHexString(signature[0]) + ";" + Utils.bytesToHexString(signature[1]) + ";" + Utils.bytesToHexString(signature[2]);
    }

    public static byte[][] stringToSignature(String signature) {
        byte[][] raw = new byte[3][];
        String[] parts = signature.split(";");
        raw[0] = Utils.hexStringToByteArray(parts[0]);
        raw[1] = Utils.hexStringToByteArray(parts[1]);
        raw[2] = Utils.hexStringToByteArray(parts[2]);
        return raw;
    }
}
